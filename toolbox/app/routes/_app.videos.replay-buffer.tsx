...
export const loader = async ({request}: LoaderFunctionArgs) => {
  ...
  const loadVideos = (run: string | null) => {
    ...
    const directoryPath = path.join(process.cwd(), "./public/vids", run, "trajectories");
...
const ReplayBufferVideoCard = ({ videoSrc, poster, startIndex, endIndex, step }: {
  videoSrc: string,
  poster: string,
  startIndex: number,
  endIndex: number,
...
export default function Page() {
  ...
  const [visibleRun1Videos, setVisibleRun1Videos] = useState(run1Videos.slice(0, windowSize));
  const [visibleRun2Videos, setVisibleRun2Videos] = useState(run2Videos.slice(0, windowSize));
  ...
  return (
    <div className="container mx-auto py-8 px-8">
      <h1 className="text-2xl font-bold mb-4">Replay Buffer Videos Comparison</h1>
      <Slider
        className="mb-8"
        min={0}
        max={Math.max(0, Math.max(run1Videos.length, run2Videos.length) - windowSize)}
