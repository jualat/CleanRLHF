...
@app.route("/videos/<path:filename>")  # Change <filename> to <path:filename>
...
#     """
#     final_path = os.path.join(base, *paths)
#     if not os.path.commonprefix([os.path.abspath(final_path), os.path.abspath(base)]) == os.path.abspath(base):
#         raise ValueError("Attempted directory traversal attack detected.")
#     return final_path
#
#
...
#     """Endpoint to serve the video files."""
#     try:
#         # Safely join the VIDEO_FOLDER with the nested filename
#         full_path = safe_join(VIDEO_FOLDER, filename)
#         directory, file_name = os.path.split(full_path)
#         return send_from_directory(directory, file_name)
#     except Exception as e:
#         logging.error(f"Failed to fetch video: {e}")
...
#                         }
#                     ), 200
#         return jsonify({"status": "error", "message": "No videos available in any environment"}), 404
#
#
#
# @app.route("/api/submit_feedback/<env_id>", methods=["POST"])
